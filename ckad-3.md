...
# Multi-Container Pod Summary

Multi-container pods in Kubernetes allow adding additional containers to a pod rather than recreating entire images to add functionality like shells or logging agents. Key points include:

- Containers within a pod should remain transient and decoupled
- All containers in a pod share the same IP address, namespace, and storage
- Kubernetes doesn't provide storage locking between containers, requiring careful configuration to prevent conflicts
- Options for managing concurrent storage access include read-only/write-only container configurations, using different directories, or implementing application-level locking
- Common multi-container pod patterns (ambassador, adapter, sidecar) are conceptual terms describing purpose rather than specific settings - all function as multi-container pods

This approach maintains scalability while providing necessary tools and functionality through complementary containers.

# Multi-Container Pod Patterns Summary

The image describes the three common patterns for secondary containers in Kubernetes multi-container pods:

1. **Ambassador**: Functions as a proxy to communicate with external resources outside the cluster. Useful when you're uncertain about the cluster configuration, letting you embed your own proxy (like Envoy) rather than using cluster-provided ones.

2. **Adapter**: Modifies data generated by the primary container to ensure proper formatting or compatibility. For example, it can transform data streams with different standards (like Microsoft's ASCII variant) into formats suitable for other systems.
